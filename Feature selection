...
data_X = pd.read_csv('C:\secom.data.txt')
X = data_X.values

#create dependent variable vector
data_y = pd.read_csv('C:\secom_labels.data.txt')
...
lsvc = LinearSVC(C=0.01, penalty="12", dual=False).fit(X, y)
...
X_train, X_test, y_train, y_test = train_test_split(X_lsvc, y, test_size = 0.4, random_state = 0)

# fitting logistic regression to Training Set

classifier = LogisticRegression(random_state = 0) 
classifier.fit(X_train, y_train)
classifier.options.mode.chained_assignment= None;
...
print.options.mode.chained_assignment= None;
